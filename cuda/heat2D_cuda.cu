...
int NX = 500;               // Grid points in x
int NY = 500;               // Grid points in y
...
__host__ __device__ inline int index2d(int i, int j, int ny) {
    return i * ny + j;
}
...
__global__ void initialize_kernel(double* T, int nx, int ny, double dx, double dy) {
    ...
    T[index2d(i, j, ny)] = 100.0 * sin(PI * x) * sin(PI * y);

    // Boundary conditions: all edges = 0
    if (i == 0 || i == nx - 1 || j == 0 || j == ny - 1) {
        T[index2d(i, j, ny)] = 0.0;
    }
}
...
__global__ void heat_step_kernel(const double* __restrict__ T_old,
    ...
    if (i == 0 || i == nx - 1 || j == 0 || j == ny - 1) {
        T_new[index2d(i, j, ny)] = 0.0;
        return;
    }

    // 2D FTCS stencil
    T_new[index2d(i, j, ny)] = T_old[index2d(i, j, ny)]
        + rx * (T_old[index2d(i+1, j, ny)] - 2.0 * T_old[index2d(i, j, ny)] + T_old[index2d(i-1, j, ny)])
...
double calculate_error(const double* T, double t, int nx, int ny,
    ...
    for (int i = 0; i < nx; i++) {
        ...
        for (int j = 0; j < ny; j++) {
            double y = j * p.dy;
            double diff = T[index2d(i, j, ny)] - analytical_solution(x, y, t);
            error += diff * diff;
        }
    }
...
void save_results(const double* T, double t, int nx, int ny,
                  const SimParams& p, const char* filename) {
...
void save_summary(double final_time, double exec_ms, double rmse,
                  double max_temp, const char* filename) {
    std::ofstream file(filename);
    file << "Final_time,Execution_time_ms,RMSE_Error,Max_temperature,NX,NY\n";
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "  Spatial step dx = " << p.dx << " m\n";
    std::cout << "  Spatial step dy = " << p.dy << " m\n";
    std::cout << "  Time step dt    = " << p.dt  << " s\n";
    std::cout << "  Diffusivity a   = " << ALPHA << " m^2/s\n";
    std::cout << "  Diffusion rx    = " << p.rx << "\n";
    std::cout << "  Diffusion ry    = " << p.ry << "\n";
    std::cout << "  rx + ry         = " << (p.rx + p.ry) << " (must be <= 0.5)\n";
    ...
    // ---- Initialize on GPU ----
    initialize_kernel<<<gridDim, blockDim>>>(d_T_old, NX, NY, p.dx, p.dy);
    CUDA_CHECK(cudaDeviceSynchronize());

    int num_steps = p.steps;
    ...
    for (int step = 0; step < num_steps; step++) {
        heat_step_kernel<<<gridDim, blockDim>>>(d_T_old, d_T_new, NX, NY, p.rx, p.ry);
    ...
    double error = calculate_error(h_T, t, NX, NY, p);
